...
const Header: React.FC = () => {
  ...
  const navLinks = [
    ...
    { href: '#sponsors', label: t('nav.sponsors'), icon: <Award /> },
  ];
  ...
  return (
    <>
      ...
      <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-[90] transition-transform duration-500 ${isMenuOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="bg-white/90 dark:bg-black/95 backdrop-blur-2xl border-t border-gray-200/50 dark:border-white/10 p-4 flex justify-around items-center">
          {navLinks.map(link => (
