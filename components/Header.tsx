...
const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${isScrolled ? 'py-3' : 'py-6'}`}>
      ...
      {isMenuOpen && (
        <div className="lg:hidden fixed inset-0 bg-white/95 dark:bg-black/95 backdrop-blur-xl z-[60] p-10 flex flex-col items-center justify-center gap-8 animate-fade-in">
          <button onClick={() => setIsMenuOpen(false)} className="absolute top-10 right-10 text-3xl"><X /></button>
          {navLinks.map(link => (
          ...
          <div className="flex gap-6 mt-10">
             <button onClick={toggleLanguage} className="text-xl font-black uppercase text-brand-green">{language}</button>
             <button onClick={toggleTheme} className="text-xl">{theme === 'light' ? <Moon /> : <Sun />}</button>
          </div>
        </div>
      )}
    </header>
